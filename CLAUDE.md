...
От него отклоняемся по фазе. Переключает уровень **человек** командой `/effort` —
Claude Code не позволяет менять effort из хука (это read-only-контекст), поэтому
полного автопереключения нет.
...
Механизм — `UserPromptSubmit`-хук `.claude/hooks/effort_advisor.py`: распознаёт фазу
по тексту промпта и печатает напоминание с рекомендованным `/effort` и причиной
(человеку — `systemMessage`, модели — `additionalContext`). Один и тот же совет не
повторяется внутри сессии. Хук советует, но не переключает — финальное решение за
человеком. `max` нельзя задать в `settings.json` (только через `/effort max` или env
