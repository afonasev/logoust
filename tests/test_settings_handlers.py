...
def test_render_settings_shows_subscription_presets():
    ...
    m = load_messages(DEFAULT_MESSAGES_PATH).settings
    assert "🎫 Варианты абонемента: 4,8,12" in render_settings(specialist, m)
...
async def test_apply_subscription_presets_valid(
    ...
    await _seed_specialist(session_factory)
    h = _handlers(messages, session_factory)
    # Whitespace and order are canonicalised on save.
    msg = _fake_message("12, 4, 8")
    state = _state(state=EditSetting.subscription_presets)
    await h.apply_subscription_presets(msg, state, _SP)
    assert _texts(msg.answer)[0] == messages.settings.saved
    async with session_factory() as session:
        updated = await get_settings(SqlAlchemySpecialistsRepo(session), _SP)
    assert updated is not None
    assert updated.subscription_presets == "4,8,12"


async def test_apply_subscription_presets_invalid_reasks(
    messages: BotMessages, session_factory: async_sessionmaker[AsyncSession]
):
    await _seed_specialist(session_factory)
    h = _handlers(messages, session_factory)
    msg = _fake_message("4,abc,8")
    state = _state(state=EditSetting.subscription_presets)
    await h.apply_subscription_presets(msg, state, _SP)
    assert _texts(msg.answer)[0] == messages.settings.bad_subscription_presets
    assert state.state == EditSetting.subscription_presets
